...
my $partsdir = $ENV{APP_ROOT} . '/PARTS/';

my $infile = $partsdir . $query . ".xml";
...
my $wiki_desc_file = $ENV{APP_ROOT}  . "/Descriptions/" . $query . ".txt";
if (-e "$wiki_desc_file") {open (WIKI_DESC, $wiki_desc_file); $wiki_description = <WIKI_DESC>; print "\nWiki Description : $wiki_description\n\n"; close WIKI_DESC;}
}

sub check_wiki_design_notes {
my $wiki_design_notes_file = $ENV{APP_ROOT}  . "/Design_Notes/" . $query . ".txt";
if (-e "$wiki_design_notes_file") {open (WIKI_DES, $wiki_design_notes_file); $wiki_design_notes = <WIKI_DES>; print "\nWiki Design Notes : $wiki_design_notes \n\n"; close WIKI_DES;}
}

sub check_wiki_sources {
my $wiki_sources_file = $ENV{APP_ROOT}  . "/Sources/" . $query . ".txt";
if (-e "$wiki_sources_file") {open (WIKI_SOURCES, $wiki_sources_file); $wiki_sources = <WIKI_SOURCES>; print "\nWiki Source Notes : $wiki_sources \n\n"; close WIKI_SOURCES;}
}